...
export default function ChatRoomPage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  // Scroll suave para o rodapé do chat
  ...
  // Carrega as mensagens e atualiza o status do pedido em segundo plano
  ...
  const handleSendText = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTextInput(content); // Devolve o texto caso falhe
    } finally {
      setSending(false);
    }
  };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Faz o upload local via Route Handler
      ...
      // Salva a mensagem como imagem no banco
  ...
  return (
    <div className="h-screen overflow-hidden bg-[#fefccf]/10 flex flex-col font-sans">
      
      {/* Lightbox Modal */}
      {lightboxImage && (
        <div className="fixed inset-0 bg-black/95 z-999 flex items-center justify-center animate-in fade-in duration-300">
          <button 
      ...
      <div className="grow overflow-y-auto max-w-[1000px] w-full mx-auto p-4 md:p-6 space-y-6 flex flex-col">
        {messages.length === 0 ? (
          /* Empty Chat Room Welcome */
      ...
      <div className="p-4 bg-[#fefccf]/20 backdrop-blur-md border-t border-slate-200/50 shrink-0">
        <div className="max-w-[800px] mx-auto">
          {isCompleted ? (
            /* Concluded banner, input blocked */
            ...
            <form onSubmit={handleSendText} className="flex items-center gap-2 bg-white rounded-2xl shadow-lg border border-slate-100 p-2">
              ...
              <button
                ...
                disabled={sending || uploading}
                className="text-slate-400 hover:text-[#f7941d] disabled:opacity-50 p-2.5 rounded-xl hover:bg-slate-50 transition-colors bg-transparent border-none cursor-pointer shrink-0"
                title="Enviar imagem"
              >
                {uploading ? (
